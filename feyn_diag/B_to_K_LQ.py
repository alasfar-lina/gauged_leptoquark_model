...
bos1 = Photon(loop_in, loop_out).bend(+1.5).addLabel(r"$V^{+}$",size= pyx.text.size.Huge )
f_loop = Fermion(loop_in, loop_out).bend(-1.5).addArrow() \
         .addLabel(r"\Ptau,\,\Pmu",displace =+0.5)
bos2 = Photon(f_loop.fracpoint(0.5), vtx).addLabel(r"\Pphoton/U", displace=0.5,size= pyx.text.size.Huge ) #.bend(0.5)
f3 = Fermion(vtx, outl).addArrow().addLabel(r"\Ptauon,\Pmuon", displace=0.3,size= pyx.text.size.Huge )
f4 = Fermion(outal,vtx).addArrow().addLabel(r"\APtauon,\APmuon", displace=0.3,size= pyx.text.size.Huge )
