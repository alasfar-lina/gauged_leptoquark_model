...
f2 = Fermion(loop_in, out1a).addArrow().addLabel(r"\Pcharm",size= pyx.text.size.Huge )
...
bos = Photon(loop_in, vtx).addLabel(r"\PWminus", displace=0.5,size= pyx.text.size.Huge ) #.bend(0.5)
f3 = Fermion(vtx, outl).addArrow().addLabel(r"\Plepton",size= pyx.text.size.Huge )
f4 = Fermion(outal,vtx).addArrow().addLabel(r"\APnu",size= pyx.text.size.Huge )
